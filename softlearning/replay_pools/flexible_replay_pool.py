...
class FlexibleReplayPool(ReplayPool):
    ...
    def add_samples(self, samples):
        ...
        index = (
            np.arange(self._pointer, self._pointer + num_samples) % self._max_size
        )  # solution for not going beyond max size
        ...
        for field_name in self.field_names:
            ...
            if (
                field_name not in samples.keys()
                and field_name in samples["infos"][0].keys()
            ):
                values = np.expand_dims(
                    np.array(
                        [
                            samples["infos"][i].get(field_name, default_value)
            ...
            try:
                ...
                if isinstance(values[0], dict):
                    values = np.stack(
                        [
                            np.concatenate(
                                [value[key] for key in value.keys()], axis=-1
                            )
                            for value in values
                        ]
                    )
    ...
    def last_n_batch(self, last_n, field_name_filter=None, **kwargs):
        last_n_indices = (
            np.arange(self._pointer - min(self.size, last_n), self._pointer)
            % self._max_size
        )
        return self.batch_by_indices(
    ...
    def filter_fields(self, field_names, field_name_filter):
        ...
        filtered_field_names = [
            field_name for field_name in field_names if filter_fn(field_name)
        ]
    ...
    def batch_by_indices(self, indices, field_name_filter=None):
        if np.any(indices % self._max_size > self.size):
            raise ValueError(
                "Tried to retrieve batch with indices greater than current" " size"
            )
        ...
        return {
            field_name: self.fields[field_name][indices] for field_name in field_names
        }
        ...
        if state["_size"] < state["_max_size"]:
            ...
            for field_name in state["fields"].keys():
                ...
                state["fields"][field_name] = np.concatenate(
                    (state["fields"][field_name], np.zeros((pad_size, *field_shape))),
                    axis=0,
                )
