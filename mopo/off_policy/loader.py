...
def restore_pool(replay_pool, experiment_root, max_size, save_path=None):
    restore_pool_ray(replay_pool, experiment_root, max_size)
    # else if "d4rl" in experiment_root:
...
# add restore for ray's datasets
def restore_pool_ray(replay_pool, experiment_root, max_size):
    from ray.rllib.offline import JsonReader

    dataList = getListOfFiles(experiment_root)
    reader = JsonReader(dataList)
    while replay_pool.size < max_size:
        ...
        batch["actions"] = (
            batch["actions"].reshape((batch["actions"].size, 1)) * 2 - 1
        )  # New shape should be (size, action_shape) --- The *2+1 is so that the "no action" doesn't become zero and kills the signal
        batch["rewards"] = batch["rewards"].reshape((batch["rewards"].size, 1))
        batch["terminals"] = batch["terminals"].reshape((batch["terminals"].size, 1))
        print("Collected Batch for pool")
        replay_pool.add_samples(batch)
...
def restore_pool_softlearning(replay_pool, experiment_root, max_size, save_path=None):
    print(
        "[ mopo/off_policy ] Loading SAC replay pool from: {}".format(experiment_root)
    )
    experience_paths = [
        checkpoint_dir
        for checkpoint_dir in sorted(
            glob.iglob(os.path.join(experiment_root, "checkpoint_*"))
        )
    ]
    ...
    for epoch in checkpoint_epochs:
        fullpath = os.path.join(
            experiment_root, "checkpoint_{}".format(epoch), "replay_pool.pkl"
        )
    ...
    if save_path is not None:
        ...
        print(
            "[ mopo/off_policy ] Replay pool average return is {}, buffer_max is {}, buffer_min is {}".format(
                avg_return, buffer_max, buffer_min
            )
        )
        d_stat = dict(
            avg_return=avg_return, buffer_max=buffer_max, buffer_min=buffer_min
        )
...
def restore_pool_bear(replay_pool, load_path):
    ...
    print(
        "[ mopo/off_policy ] {} trajectories | avg return: {}".format(
            num_trajectories, avg_return
        )
    )
...
def restore_pool_contiguous(replay_pool, load_path):
    print(
        "[ mopo/off_policy ] Loading contiguous replay pool from: {}".format(load_path)
    )
    import numpy as np
    ...
    assert expected_dim == actual_dim, "Expected {} dimensions, found {}".format(
